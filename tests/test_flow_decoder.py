...
def flow_model_linear():
    return FlowDecoder(latent_dim=LDIM, token_dim=TDIM, image_size=ISIZE,
                       n_channels=N_CH, n_blocks=N_BLK, path_type='linear')


@pytest.fixture(scope="module")
def flow_model_no_token():
    return FlowDecoder(latent_dim=LDIM, token_dim=0, image_size=ISIZE,
                       n_channels=N_CH, n_blocks=N_BLK)
...
def test_sample_guidance(flow_model_linear):
    model = FlowDecoder(latent_dim=LDIM, token_dim=TDIM, image_size=ISIZE,
                        n_channels=N_CH, n_blocks=N_BLK, use_guidance=True)
    lat = torch.randn(BATCH, LDIM)
    tok = torch.randn(BATCH, TDIM)
...
def test_snr_conditioning_shapes():
    model = FlowDecoder(latent_dim=LDIM, token_dim=TDIM, image_size=ISIZE,
                        n_channels=N_CH, n_blocks=N_BLK, snr_conditioning=True)
    lat     = torch.randn(BATCH, LDIM)
    tok     = torch.randn(BATCH, TDIM)
...
def test_ot_path_loss():
    ...
    model = FlowDecoder(latent_dim=LDIM, token_dim=TDIM, image_size=ISIZE,
                        n_channels=N_CH, n_blocks=N_BLK, path_type='ot')
    x0   = torch.randn(BATCH, 3, ISIZE, ISIZE)
    lat  = torch.randn(BATCH, LDIM)
    tok  = torch.randn(BATCH, TDIM)
