...
class DiffusionDecoder(nn.Module):
    def __init__(self, latent_dim: int, token_dim: int, image_size: int,
                 conditioning: str = 'concat', base_channels: int = 128,
                 n_levels: int = 3, timesteps: int = 1000,
                 use_vae_latent: bool = False, snr_conditioning: bool = False):
        super().__init__()
        ...
        if conditioning in ['cross_attention', 'concat', 'none', 'adain']:
            self.unet = UNet2DConditionModel(
                ...
                layers_per_block=2,
                block_out_channels=(base_channels, base_channels*2, base_channels*2) if n_levels==3 else (base_channels, base_channels*2, base_channels*2, base_channels*4),
                down_block_types=(
                    "CrossAttnDownBlock2D" if conditioning == 'cross_attention' else "DownBlock2D",
                    "CrossAttnDownBlock2D" if conditioning == 'cross_attention' else "DownBlock2D",
                    "DownBlock2D"
                ) if n_levels==3 else ("DownBlock2D",)*n_levels,
                up_block_types=(
                    "UpBlock2D",
                    "CrossAttnUpBlock2D" if conditioning == 'cross_attention' else "UpBlock2D",
                    "CrossAttnUpBlock2D" if conditioning == 'cross_attention' else "UpBlock2D"
                ) if n_levels==3 else ("UpBlock2D",)*n_levels,
                cross_attention_dim=cond_dim if conditioning == 'cross_attention' else None
            )
    ...
    def compute_loss(self, x0, latent, token, snr_emb=None):
        ...
        if self.conditioning == 'concat':
            cond_spatial = cond.view(B, -1, 1, 1).expand(-1, -1, x_t.shape[2], x_t.shape[3])
    ...
    def sample(self, latent, token, steps=50, sampler='ddim',
        ...
        for i, t_cur in enumerate(timesteps):
            ...
            def get_eps(in_x, t_, c_):
                if self.conditioning == 'concat':
                    c_sp = c_.view(B, -1, 1, 1).expand(-1, -1, in_x.shape[2], in_x.shape[3])
            ...
            alpha_bar_prev = self.alpha_bar[timesteps[i+1]] if i+1 < steps else torch.tensor(1.0, device=x.device)
