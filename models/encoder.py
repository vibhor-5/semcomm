...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        if encoder_type == "cnn":
            ...
            mobilenet.classifier[1] = nn.Linear(
                mobilenet.classifier[1].in_features, latent_dim
            )
        ...
        if semantic_token_type == "clip":
            ...
            self.clip_model, self.clip_preprocess = clip.load(
                "ViT-B/32", device=device if device else "cpu"
            )
    ...
    def forward(self, image: Tensor) -> tuple[Tensor, Tensor]:
        ...
        if (
            self.semantic_token_type == "clip"
            and getattr(self, "clip_model", None) is not None
        ):
            with torch.no_grad():
