...
class Trainer:
    ...
    def _train_step(self, batch) -> dict:
            ...
            if self.loss_cfg.get("clip_weight", 0) > 0:
                with torch.no_grad():
                    recon = self.decoder.sample(
                        noisy_latent, tokens, steps=10, snr_emb=snr_emb
                    )
                clip_loss = (
                    self.clip_loss_fn(images, recon) * self.loss_cfg["clip_weight"]
                )
        ...
        grad_norm = torch.nn.utils.clip_grad_norm_(
            all_params, self.cfg["training"]["gradient_clip"]
        )
    ...
    def _validate(self, val_loader):
        ...
        with torch.no_grad():
            for i, batch in enumerate(val_loader):
                ...
                if self.cfg.get("channel", {}).get("snr_conditioning", False):
                    noisy_latent, snr_used = self.channel(latent)
                    snr_emb = torch.full(
                        (images.shape[0], 1), snr_used, device=self.device
                    )
                else:
                    noisy_latent = self.channel(latent)
                    snr_emb = None
        ...
        if hasattr(self, "clip_loss_fn"):
            ...
            def encode_clip(x):
                x = F.interpolate(
                    x.to(self.device), size=224, mode="bilinear", align_corners=False
                )
                x = (x + 1.0) / 2.0
                x = normalise_fn(x)
    ...
    def run(self, train_loader, val_loader, n_epochs):
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            mean_train = {
                k: float(np.mean([d[k] for d in train_losses])) for k in train_losses[0]
            }
