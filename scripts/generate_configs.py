...
for path_type in ['linear', 'ot']:
    eid = f'B1_{path_type}'
    cfg = flow_cfg(eid, f'Flow Path Compare — {path_type.upper()}',
...
for n in [3, 5, 8]:
    eid = f'B2_nblocks{n}'
    cfg = flow_cfg(eid, f'CNF Depth Ablation n_blocks={n}',
...
for variant, enc_override in B3_CONDS.items():
    eid = f'B3_{variant}'
    cfg = flow_cfg(eid, f'Semantic Conditioning — {variant}',
...
for k in [32, 64, 128, 256]:
    eid = f'B4_k{k}'
    cfg = flow_cfg(eid, f'Rate-Semantic Curve Flow k={k}',
                   {'encoder': {'latent_dim': k}, 'training': {'n_epochs': 100}})
    write_config(cfg, f'configs/flow/{eid}.yaml')

# B5 — SNR sweep (eval only, no training)
eid = 'B5_snr_sweep'
cfg = flow_cfg(eid, 'Channel Robustness SNR Sweep (Flow — eval only)',
               {'training': {'n_epochs': 0}})
...
for variant in ['random_snr', 'random_snr_token']:
    ...
    eid = f'B6_{variant}'
    cfg = flow_cfg(eid, f'Channel-Aware Flow Training — {variant}',
                   {
                       'channel': {'snr_db': None, 'snr_range': [0, 20],
...
for w in [1.0, 1.5, 2.0, 3.0]:
    ...
    cfg = flow_cfg(eid, f'Guidance Weight Sweep w={w} (Flow — eval only)',
                   {
...
eid = 'B8_tinyimagenet'
cfg = flow_cfg(eid, 'Scale-Up TinyImageNet 64×64 (Flow)',
               {
                   'dataset': {
                       'name': 'tinyimagenet',
                       'image_size': 64,
...
for steps in [5, 10, 25, 50, 100, 200]:
    ...
    cfg = flow_cfg(eid, f'ODE Steps vs Quality steps={steps} (eval only)',
                   {
...
for ch_type in ['awgn', 'rayleigh', 'burst']:
    eid = f'B10_{ch_type}'
    cfg = flow_cfg(eid, f'Fading Channel Robustness — {ch_type} (eval only)',
                   {
...
for cond in ['concat', 'cross_attention', 'adain', 'none']:
    eid = f'C1_{cond}'
    cfg = diff_cfg(eid, f'Conditioning Strategy — {cond}',
                   {
...
for steps in [5, 10, 25, 50, 100, 200]:
    ...
    cfg = diff_cfg(eid, f'Diffusion Steps vs Quality steps={steps} (eval only)',
                   {
                       'evaluation': {'sampling_steps': steps},
                       'training': {'n_epochs': 0},
                   })
...
for k in [32, 64, 128, 256]:
    eid = f'C3_k{k}'
    cfg = diff_cfg(eid, f'Rate-Semantic Curve Diffusion k={k}',
                   {
...
for w in [1.0, 1.5, 2.0, 3.0, 5.0]:
    ...
    cfg = diff_cfg(eid, f'Guidance Weight Sweep w={w} (Diffusion — eval only)',
                   {
                       'evaluation': {'guidance_scale': w},
                       'training': {'n_epochs': 0},
                   })
...
eid = 'C5_snr_sweep'
cfg = diff_cfg(eid, 'Channel Robustness SNR Sweep (Diffusion — eval only)',
               {'training': {'n_epochs': 0}})
...
for variant in ['random_snr', 'random_snr_token']:
    use_token = variant == 'random_snr_token'
    eid = f'C6_{variant}'
    cfg = diff_cfg(eid, f'Channel-Aware Diffusion Training — {variant}',
                   {
                       'channel': {'snr_db': None, 'snr_range': [0, 20],
                                   'snr_conditioning': use_token},
                       'decoder': {'snr_conditioning': use_token},
...
for space in ['pixel', 'vae_latent']:
    ...
    eid = f'C7_{space}'
    cfg = diff_cfg(eid, f'Diffusion Space — {space}',
                   {
...
for variant, loss_override in LOSS_VARIANTS.items():
    eid = f'C8_{variant}'
    cfg = diff_cfg(eid, f'Semantic Loss Augmentation — {variant}',
                   {'loss': loss_override, 'training': {'n_epochs': 80}})
    write_config(cfg, f'configs/diffusion/{eid}.yaml')

# C9 — TinyImageNet scale-up
eid = 'C9_tinyimagenet'
cfg = diff_cfg(eid, 'Scale-Up TinyImageNet 64×64 (Diffusion)',
               {
                   'dataset': {
                       'name': 'tinyimagenet',
                       'image_size': 64,
...
for ch_type in ['awgn', 'rayleigh', 'burst']:
    eid = f'C10_{ch_type}'
    cfg = diff_cfg(eid, f'Fading Channel Robustness — {ch_type} (Diffusion eval only)',
                   {
                       'channel': {'type': ch_type,
                                   'erasure_prob': 0.2 if ch_type == 'burst' else None},
                       'training': {'n_epochs': 0},
...
for enc_type in ['cnn', 'resnet18', 'mobilenetv2']:
    eid = f'E1_{enc_type}'
    cfg = flow_cfg(eid, f'Encoder Architecture — {enc_type}',
                   {
...
for tok_type in ['clip', 'learned', 'class_onehot', 'none']:
    eid = f'E2_{tok_type}'
    cfg = flow_cfg(eid, f'Semantic Token Type — {tok_type}',
                   {
...
for quant_method in ['uniform8', 'uniform4', 'vqvae', 'none']:
    ...
    cfg = flow_cfg(eid, f'Quantisation Method — {quant_method}',
                   {
...
for aug in ['none', 'basic', 'full', 'mixup']:
    eid = f'E4_{aug}'
    cfg = flow_cfg(eid, f'Data Augmentation Impact — {aug}',
                   {
