...
def build_channel(channel_cfg):
    ...
    if ctype == 'awgn':
        ...
        return RayleighChannel(snr_db=snr_db, snr_range=snr_range, return_snr=return_snr)
    elif ctype == 'burst':
...
def append_summary_csv(output_dir: str, row: dict):
    ...
    fieldnames = [
        'experiment_id', 'clip', 'lpips', 'fid', 'psnr', 'ssim',
        'bpp', 'sampling_time_ms', 'n_epochs_trained', 'gpu', 'date',
    ]
...
def main():
    ...
    parser.add_argument('--resume',     action='store_true',
                        help='Resume from the latest saved checkpoint')
    ...
    val_split = ('test' if cfg['dataset']['name'] in ('cifar10', 'cifar100')
                 else 'val')

    train_ds = SemCommDataset(
        args.data_dir, 'train', cfg['dataset']['name'],
        cfg['dataset']['image_size'],
        clip_cache_path = clip_cache,
        transform       = train_trans.get(aug_name, train_trans['basic']),
    )
    val_ds = SemCommDataset(
        args.data_dir, val_split, cfg['dataset']['name'],
        cfg['dataset']['image_size'],
        clip_cache_path = clip_cache,
        transform       = val_trans,
    )
    ...
    if not args.eval_only and n_epochs > 0:
        trainer = Trainer(encoder, channel, decoder,
                          cfg['loss'], cfg, drive_root=args.output_dir)
    ...
    if n_epochs > 0:
        ...
        if best_ckpt and os.path.exists(best_ckpt):
            ...
            metrics = run_evaluation(
                cfg, best_ckpt, val_loader, device,
            ...
            append_summary_csv(args.output_dir, {
